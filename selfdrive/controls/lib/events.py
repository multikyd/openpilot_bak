...
def standstill_alert(CP, sm, metric):
  ...
  if elapsed_time_min == 0:
    return Alert(
      "STANDSTILL (Time Elapsed: 00:%02d)" % (elapsed_time_sec),
      "",
      AlertStatus.normal, AlertSize.small,
      Priority.LOW, VisualAlert.none, AudibleAlert.none, .0, .1, .1, alert_rate=0.5)
  else:
    return Alert(
      "STANDSTILL (Time Elapsed: %02d:%02d)" % (elapsed_time_min, elapsed_time_sec),
      "",
      AlertStatus.normal, AlertSize.small,
      Priority.LOW, VisualAlert.none, AudibleAlert.none, .0, .1, .1, alert_rate=0.5)
...
EVENTS: Dict[int, Dict[str, Union[Alert, Callable[[Any, messaging.SubMaster, bool], Alert]]]] = {
  ...
  EventName.laneChangeManual: {
    ET.WARNING: Alert(
      "Lane Changing Manually",
      "Steering Temporarily Disabled Take Control",
      AlertStatus.userPrompt, AlertSize.mid,
      Priority.LOW, VisualAlert.none, AudibleAlert.none, .0, .1, .1, alert_rate=0.75),
  },
  ...
  EventName.driverSteering: {
    ET.WARNING: Alert(
      "Driver Handling Steering",
      "Steering Temporarily Compromised",
      AlertStatus.userPrompt, AlertSize.mid,
      Priority.LOW, VisualAlert.none, AudibleAlert.none, .0, .1, .1),
  },
